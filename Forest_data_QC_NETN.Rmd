...
params:
  file: "NETN_tree_sapling_data.csv"
---
...
library(knitr)
library(kableExtra)
#dat <- read.csv("./data/NETN_tree_sapling_data.csv")
...
### Tree Heights {.tabset}
Checking height codes that vary more than 10% among years within the same plot. NETN methods changed over time (see below), so a bit tricky to QC. 

History of tree height in NETN:
* 2006-2008: Only measured height of one codominant tree per plot, and didn't record tree tag.
...
From the table/plots, it's clear that most of the rogue measurements are from the first cycle. At the time we used a clinometer, then manually calculated heights as we entered the data. We also didn't have measurements to compare against, like we did in later surveys. It's tempting to throw out the first cycle of heights an instead use an average of heights collected after 2010. 

```{r results = 'hide'}
hts <- 
  rbind(dat |> filter(year < 2011) |> filter(treeclcd == 3) |> select(park, plt_cn, year, cycle, treeclcd, ht) |> unique(), # early data
...
acad <- ggplot(hts_out2 |> filter(plt_cn %in% codom_outs) |> filter(park == "ACAD"), 
               aes(x = cycle, y = hts_avg)) +
        geom_line() + facet_wrap(~plt_cn) + theme_bw() +
        labs(x = "Cycle", y = "Tree Ht (m)") +
        theme(strip.text = element_text(size = 7))

nhp <- ggplot(hts_out2 |> filter(plt_cn %in% codom_outs) |> filter(!park == "ACAD"), 
               aes(x = cycle, y = hts_avg)) +
       geom_line() + facet_wrap(~plt_cn) + theme_bw() +
        labs(x = "Cycle", y = "Tree Ht (m)") +
        theme(strip.text = element_text(size = 7))
...
#### Outliers +/- 50%
NETN plots with at least 1 identified outlier (+/- 50% of median) for codominant trees. 
...
#### ACAD less extreme outs
ACAD plots with at least 1 identified outlier for codominant trees as being outside 99% quantile of heights measured within a given plot.
```{r fig.height = 10, out.width = '100%'}
acad
```

#### NHP less extreme outs
Non-ACAD (National Historic Parks) plots with at least 1 identified outlier for codominant trees as being outside 99% quantile of heights measured within a given plot.
```{r fig.height = 10, out.width = '100%'}
nhp
```
